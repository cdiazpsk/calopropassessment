...
export function scoreAssessment(form){
  ...
  const base=score<70?8:category==='Full Service'?8:10;
  let prod=isHeavy?6:base;
  if(release>0)prod=isHeavy?Math.max(6,Math.min(release,base)):Math.min(prod,release);
  let crew=(rooms>150||isHeavy||prod<=7)?'1 Lead + 3 Technicians':'1 Lead + 2 Technicians';
  const poorPaint=Number(form.paint_score||3)<=2||issues.includes('Widespread paint touch-ups')||issues.includes('Corner-to-corner paint needed');
  const poorCaulk=Number(form.caulk_score||3)<=2||issues.includes('Heavy caulking needed');
  const damageRatings=['furniture_score','hvac_score','plumbing_score','doors_score'];
  const broadDamage=damageRatings.filter(k=>Number(form[k]||3)<=2).length>=2||issues.includes('Bathroom repairs')||issues.includes('Every-room scope varies');
  const conditionRatings=['paint_score','caulk_score',...damageRatings];
  const veryLowCondition=conditionRatings.reduce((sum,k)=>sum+Number(form[k]||3),0)/conditionRatings.length<=2.25;
  const severeRoomCondition=poorPaint&&poorCaulk&&(broadDamage||veryLowCondition);
  let program='Standard PM';
  if(severeRoomCondition)program='Heavy-Condition PM Review';
  const readinessScore=Math.max(0,Math.min(100,score));
