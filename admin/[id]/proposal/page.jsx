    ...
    <>
      ...
      <main style={{ maxWidth: 900, margin: '0 auto', padding: '36px 20px 70px' }}>
        <div style={{ marginBottom: 24 }}>
          <Link href={`/admin/${a.id}`} className="btn-secondary">Back to Assessment</Link>
        </div>
        ...
        <section className="card" style={{ padding: 34, background: 'white' }}>
          ...
          <p>Caliber Lodging has reviewed the submitted property assessment for {a.property_name}. Based on the information provided, the recommended program is <strong>{program}</strong>.</p>

          <h2 style={{ color: '#0B2F5B' }}>Recommended Scope</h2>
          <ul>
            <li>Guestroom preventative maintenance and asset preservation review</li>
            <li>Paint, caulk, furniture, HVAC/PTAC, plumbing, doors, and logistics evaluation</li>
            <li>Photo and video based review of submitted conditions</li>
            <li>Digital reporting and recommendations</li>
          </ul>

          <h2 style={{ color: '#0B2F5B' }}>Production Forecast</h2>
          <p>Estimated production cadence: <strong>{prod} rooms per day</strong>, subject to room release, final scope, and property coordination.</p>
          ...
          {addons.length ? <ul>{addons.map(x => <li key={x}>{x}</li>)}</ul> : <p>No AI add-ons have been generated yet. Run AI Assessment for recommended add-ons.</p>}

          <h2 style={{ color: '#0B2F5B' }}>Assumptions</h2>
          <ul>
            <li>Pricing is budgetary until final scope confirmation.</li>
            <li>Room release and property access directly impact production cadence.</li>
            <li>Owner-supplied materials, comp rooms, and meals may impact final pricing.</li>
            <li>Heavy deferred maintenance may require revised pricing or add-on scope.</li>
          </ul>
        </section>
      </main>
    </>
