...
export function calculateEstimate(form, readinessScore = 60) {
  ...
  if (service === 'Guestroom PM') {
    ...
    const rating = key => Number(form[key] || 3);
    const poorPaint = rating('paint_score') <= 2 || hasAny(issues, ['Widespread paint touch-ups', 'Corner-to-corner paint needed']);
    const poorCaulk = rating('caulk_score') <= 2 || issues.includes('Heavy caulking needed');
    const damageRatings = ['furniture_score', 'hvac_score', 'plumbing_score', 'doors_score'];
    const broadDamage = damageRatings.filter(key => rating(key) <= 2).length >= 2 || hasAny(issues, ['Bathroom repairs', 'Every-room scope varies']);
    const conditionRatings = ['paint_score', 'caulk_score', ...damageRatings];
    const veryLowCondition = conditionRatings.reduce((sum, key) => sum + rating(key), 0) / conditionRatings.length <= 2.25;
    const severeRoomCondition = poorPaint && poorCaulk && (broadDamage || veryLowCondition);

    const unitLow = severeRoomCondition ? 195 : 175;
    const unitHigh = severeRoomCondition ? 225 : 195;
    const tier = severeRoomCondition ? 'heavy-condition preventative maintenance' : 'standard preventative maintenance';
    return result({
      low: rooms * unitLow,
      high: rooms * unitHigh,
      unitLow,
      unitHigh,
      unit: 'room',
      basis: `${rooms} guestrooms at the ${tier} pricing range. Final proposal pricing will validate logistics, travel, access, materials, and owner-provided support.`,
      confidence: 'Moderate'
    });
  }
